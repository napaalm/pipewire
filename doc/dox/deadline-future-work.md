...
## Capability-driven node admission

The fusion-eligibility validator
(`src/modules/module-deadline/fusion_validator.c`) carries the
full structural predicate set today, but its `analyzability`
and `blocking-closure` branches are vacuous on the production
code path because `reconcile_topo_t` does not yet carry per-
follower analyzability flags (main-loop / exported / remote)
or per-plugin `FUSION_CAP_NONBLOCKING_PROCESS` capability bits.
The topology snapshot pre-filters out the unsupported cases
...
JSON snapshot's rejection-reason column genuinely informative.
...
If the blocking-closure predicate is relaxed to admit nodes
that can suspend in `process()`, the feasibility analysis must
switch from constrained-deadline EDF (the current Baruah,
Howell & Rosier 1990 model) to one of the suspension-aware
variants surveyed in Chen, Nelissen, Huang, Yang, Brandenburg,
Bletsas, Liu, Richard, Ridouard, Audsley, Rajkumar, de Niz,
von der Bruggen, "Many Suspensions, Many Problems", Real-Time
Systems 55(1):144-207, 2019. The trade-off is admission
restrictiveness versus model complexity; the current code
plays it safe by rejecting suspension and never claiming hard
guarantees for a self-suspending task.
