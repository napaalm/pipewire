...
The published schedule is in **hard mode** when *all* of:
  ...
  * every member of every fused group is assumed non-blocking
    inside its `process()` call (the contracted-DAG EDF
    feasibility proof requires non-self-suspending tasks);
