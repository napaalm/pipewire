 ...
 * analyzability flags; the topology snapshot pre-filters main-
 * loop / exported / tid<=0 followers out of the candidate set,
 * and every dynamic-loop follower is implicitly treated as
 * non-blocking inside process(). The structural predicates that
 * the current data path runs are predecessor closure, precedence
 * convexity, and externally atomic; the analyzability predicates
 * become load-bearing once the topo snapshot carries the missing
 * bits.
 */
